...
struct ReplicaInfoFrost {
    ...
    ReplicaInfoFrost(ReplicaID id,
                ...
                pubkey_bt &&pubkey):
        id(id), peer_id(peer_id), pubkey(std::move(pubkey)) {std::cout << "---- STO IN ReplicaInfoFrost riga 45 DENTRO entity.h package:include->hotstuff---- " << std::endl;
    }

    ReplicaInfoFrost(const ReplicaInfoFrost &other):
        id(other.id), peer_id(other.peer_id),
        pubkey(other.pubkey->clone()) {std::cout << "---- STO IN ReplicaInfoFrost riga 51 DENTRO entity.h package:include->hotstuff---- " << std::endl;
    }

    ReplicaInfoFrost(ReplicaInfoFrost &&other):
        id(other.id), peer_id(other.peer_id),
        pubkey(std::move(other.pubkey)) {std::cout << "---- STO IN ReplicaInfoFrost riga 56 DENTRO entity.h package:include->hotstuff---- " << std::endl;
    }
};

class ReplicaConfigFrost {
    std::unordered_map<ReplicaID, ReplicaInfoFrost> replica_map;

    public:
    size_t nreplicas;
    size_t nmajority;

    ReplicaConfigFrost(): nreplicas(0), nmajority(0) {}
    ...
    const ReplicaInfoFrost &get_info(ReplicaID rid) const {
...
class BlockFrost;
class HotStuffCoreFrost;    //todo: implementarla, sta in consensus.h

using block_t = salticidae::ArcObj<BlockFrost>;

class CommandFrost: public Serializable {
    friend HotStuffCoreFrost;
    public:
    virtual ~CommandFrost() = default;
...
using command_t = ArcObj<CommandFrost>;
...
class BlockFrost {
    ...
    BlockFrost(bool delivered, int8_t decision):
    ...
    BlockFrost(const std::vector<block_t> &parents,
    ...
    void unserialize(DataStream &s, HotStuffCoreFrost *hsc);
    ...
    bool verify(const HotStuffCoreFrost *hsc) const;

    promise_t verify(const HotStuffCoreFrost *hsc, VeriPool &vpool) const;
...
class EntityStorageFrost {
    ...
    block_t add_blk(BlockFrost &&_blk, const ReplicaConfigFrost &/*config*/) {
        ...
        //    return nullptr;
        //}
        block_t blk = new BlockFrost(std::move(_blk));
